...
function help {
    ...
    echo ""
    echo "Commands:"
    echo "    gitinit [branch] - initializes all git submodules and checks out the tardet branch (default = development)"
    echo "    install          - installs all application repositories"
    echo "    update [branch]  - updates all client's with latest publish @tonomy/tonomy-id-sdk repository for branch (default = development)"
    echo "    link             - links all repositories together with yarn link command, useful for debugging"
...
function printservices {
    ...
    echo "Blockchain explorer               - https://local.bloks.io/?nodeUrl=http%3A%2F%2Flocalhost%3A8888&coreSymbol=TONO&corePrecision=6&systemDomain=eosio"
    echo "Tonomy Accounts (react)           - http://${ip}:3000 "
    echo "Tonomy Demo (react)               - http://${ip}:3001"
    echo "Tonomy Apps (react)               - http://${ip}:3003 "
